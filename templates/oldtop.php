 ...
 /* segons seccio afegim css de seccio, keywords i descripcio. */
	switch ($page->section) {
		...
		case ('critiques'):
            ...
            $page->add_scripts('<script type="text/javascript" src="http://s7.addthis.com/js/250/addthis_widget.js#pubid=ra-4da62dec5b48fd17"></script>');
		...
		break;
		/*case ('croniques'):
			$page->description = 'Críticas, reviews, crónicas de conciertos y entrevistas de Sombras. Web Oficial de Sombras, banda de flolk rock de barcelonesa';
			$page->words = 'crtica, review, crónica, entrevista, prensa, '.$page->words;
			$page->title = 'Web Oficial de Sombras - Prensa - Folk Rock';
			$page->add_css('<link type="text/css" rel="stylesheet" media="all" href="css/press.css" />');			
		break;
		case ('cronicas'):
			$page->description = 'Reviews, live reports and interviews with Sombras. Sombras Official Site, Folk rock band from Barcelona';
			$page->words = 'review, live, report, interview, press, '.$page->words;
			$page->title = 'Sombras Official Site - Press - Folk Rock';
			$page->add_css('<link type="text/css" rel="stylesheet" media="all" href="css/press.css" />');			
		break;*/
		case ('entrevistes'):
...
<?php
	...
	if ($page->leng == 'ES') {
		$cad_menu = array ('inicio','noticias','conciertos','criticas','cronicas','entrevistas','contacto'); /* afegir concerts!!! */
	} else {
		$cad_menu = array ('inici','noticies','concerts','critiques','croniques','entrevistes','contacte');
	}
