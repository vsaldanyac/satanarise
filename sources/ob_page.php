<?php
    ...
    public function __set($name,$value)
		...
		public function __construct() /* inicialitza l'objecte */
		{
			...
			$this->css = array('<link type="text/css" rel="stylesheet" media="all" href="css/main.css" />',
			...
			$this->words = 'Metal, reviews, críticas, entrevistas, noticias, power, viking, death, heavy, trash, black, gothic, gotico, progresive, progresivo';
			$this->scripts=array();
			$this->punter=1;
			$this->quantitat=10;
      ...
      print '<meta property="og:image" content="http://www.satanarise.com/'.$this->image_main.'" />';
		}
	...
	/* control de seccio, si la seccio del parametre no existeix en funcio de l'idioma sera news o noticias */
			...
			if (($this->leng == 'CAT') && (($this->section != 'inici') && ($this->section != 'critiques') && ($this->section != 'noticies') && ($this->section != 'concerts') && ($this->section != 'croniques') && ($this->section != 'entrevistes') && ($this->section != 'entrevistesn') && ($this->section != 'opinio') && ($this->section != 'contacte') && ($this->section != 'pagina') && ($this->section != 'concurs') && ($this->section != 'carteslector') && ($this->section != 'preferits') && ($this->section != 'link'))) 
			{
				$this->section = 'inici';
			}
			if (($this->leng == 'ES') && (($this->section != 'inicio') && ($this->section != 'criticas') && ($this->section != 'noticias') && ($this->section != 'conciertos') && ($this->section != 'cronicas') && ($this->section != 'entrevistas') && ($this->section != 'entrevistasn') && ($this->section != 'opinion') && ($this->section != 'contacto') && ($this->section != 'pagina') && ($this->section != 'concurso') && ($this->section != 'cartaslector') && ($this->section != 'preferidos') && ($this->section != 'link'))) 
			{
				$this->section = 'inicio';
			}   
			if (isset($param['pnt'])) 
			...
			/* parametres de la seccio noticies */
			{
				...
				if (($this->id!='') && ($this->noticia!='')) /* veure si hi ha traducció */
				{
					switch ($this->leng)
					{
						case 'ES':
					...
					$bd->conectar();
					$query="select Title from newscontent where idNews = ".$this->id." and Idioma='".$idioma."'";
          ...
          $query="select Title, Body from newscontent where idNews = ".$this->id." and Idioma='".$this->leng."'";
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
					   if ($resultat->num_rows==1)
					   {
                           ...
                           $this->noticia_descripcio=substr($row['Body'],0,200);
					   }
		      }
          $query="select ruta from newsdata where idNews = ".$this->id." and orden=1 and tipo=1";
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
					   if ($resultat->num_rows==1)
      ...
      if ($this->section=='pagina')
			...
			/* parametres de la secció concerts */
			{
				...
				if (($this->concert_tipus!='normal') && ($this->concert_tipus!='entrada') && ($this->concert_tipus!='entradas') && ($this->concert_tipus!='agenda')) 
				{
					$this->concert_tipus='normal';	
				}
				if (isset($param['id']))
				{
					$this->id=$param['id'];
					$bd->conectar();
					$query="select concerts.tipus, concerts.cartell, concerts.Nom, concertsgrups.Grup from concerts, concertsgrups where concerts.idGig = ".$this->id." and concertsgrups.idGig = ".$this->id." order by ordre";
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
						...
						switch ($row['tipus'])
						{
							case 1:
								if ($row['Nom']=='') 
								{
									...
									{
										$row=$resultat->fetch_assoc();
										$this->concerts_titol=$this->concerts_titol.' + '.$row['Grup'];
									}
								} else {
			...
			/* parametres de la secció concerts */
			{
				...
				if (($this->reviews_tipus!='main') && ($this->reviews_tipus!='entrada') && ($this->reviews_tipus!='all')) 
				{
					$this->reviews_tipus='main';	
				}
				if (isset($param['id']))
				{
                    ...
                    if ($this->leng=='ES')
                    {
                        $query="select banda, disc, any, texte_es, portada from reviews where idreviews = ".$this->id;
                    }
                    else 
                    {
                        $query="select banda, disc, any, texte_cat, portada from reviews where idreviews = ".$this->id;
                    }
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
						$num_resultats=$resultat->num_rows;
						$row=$resultat->fetch_assoc();
                        $this->review_titol='Crítica de '.$row['disc'].' de '.$row['banda'].' ('.$row['any'].')'; 
                        if ($this->leng=='ES')
                        {
                            $this->review_descripcio=substr($row['texte_es'],0,200); 
                        }
                        else 
                        {
                            $this->review_descripcio=substr($row['texte_cat'],0,200);
                        }
			...
			if ($this->leng=='ES')
            {
                $this->review_descripcio='Els musics diuen quins discos son els seus preferits';
                $this->review_titol='Preferits'; 
            }
            else
            {
                $this->review_descripcio='Los músicos opinan sobre sus discos favoritos';
                $this->review_titol='Preferidos';
            } 
			}
			
      if (($this->section=='entrevistas') || ($this->section=='entrevistes'))
			/* parametres de la secció concerts */
			{
				if (isset($param['type']))
				{
				...
				if (($this->entrevista_tipus!='main') && ($this->entrevista_tipus!='entrada')) 
				{
					$this->entrevista_tipus='main';	
				}
				if (isset($param['id']))
          ...
          if ($this->leng=='ES')
          {
              $query="select idioma, banda, titol_es from entrevistes where identrevistes = ".$this->id;
          }
          else
          {
              $query="select idioma, banda, titol_cat from entrevistes where identrevistes = ".$this->id;
          }                    
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
            ...
            if ($this->leng=='ES')
            {
                $this->entrevista_titol='Entrevista con '.$row['banda'];
                $this->entrevista_descripcio=substr($row['titol_es'],0,200); 
            }
            else
            {
                $this->entrevista_titol='Entrevista amb '.$row['banda'];
                $this->entrevista_descripcio=substr($row['titol_cat'],0,200);
            }
            $query="select ruta from entrevistesdata where identrevistes=".$this->id." and tipus=1 and ordre=1";
            $resultat=$bd->bd->query($query);
            if ($resultat!=FALSE)
            {	
            	$row=$resultat->fetch_assoc();
              $this->image_main='pics/entrevistes_pics/'.$row['ruta'];
            }
            if ($idioma_ent!='BOTH') $this->traduccio=FALSE;
					}
					$bd->desconectar();
      ...
      if (($this->section=='entrevistasn') || ($this->section=='entrevistesn'))
			/* parametres de la secció concerts */
			{
				if (isset($param['type']))
				{
          ...
          if ($this->leng=='ES')
          {
              $query="select idioma, banda, titol_es from entrevnews where identrevistes = ".$this->id;
          }
          else
          {
              $query="select idioma, banda, titol_cat from entrevinews where identrevistes = ".$this->id;
          }                    
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
            ...
            $query="select ruta from entrevnewsdata where identrevistes=".$this->id." and tipus=1 and ordre=1";
            $resultat=$bd->bd->query($query);
            if ($resultat!=FALSE)
            {	
            	$row=$resultat->fetch_assoc();
              $this->image_main='pics/entrevistes_news_pics/'.$row['ruta'];
            }
            if ($idioma_ent!='BOTH') $this->traduccio=FALSE;
					}
					$bd->desconectar();
      ...
      if (($this->section=='opinion') || ($this->section=='opinio'))
			/* parametres de la secció concerts */
			{
				...
				if (($this->opinio_tipus!='main') && ($this->opinio_tipus!='entrada')) 
				{
					$this->opinio_tipus='main';	
				}
				if (isset($param['id']))
          ...
          if ($this->leng=='ES')
          {
              $query="select idioma, titol_es, texte_es, ruta from opinio where idopinio = ".$this->id;
          }
          else
          {
              $query="select idioma, titol_cat, texte_cat, ruta from opinio where idopinio = ".$this->id;
          }                    
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
            ...
            if ($this->leng=='ES')
            {
                ...
                $this->opinio_descripcio=substr($row['texte_es'],0,200); 
            }
            else
            {
                $this->opinio_titol=$row['titol_cat'];
                $this->opinio_descripcio=substr($row['texte_cat'],0,200);
            }
            if ($idioma_ent!='BOTH') $this->traduccio=FALSE;
					}
					$bd->desconectar();
      ...
      if (($this->section=='carteslector') || ($this->section=='cartaslector'))
			/* parametres de la secció concerts */
			{
				if (isset($param['type']))
				{
				...
				if (($this->carta_tipus!='main') && ($this->carta_tipus!='entrada')) 
				{
					$this->carta_tipus='main';	
				}
				if (isset($param['id']))
				{
                    ...
                    if ($this->leng=='ES')
                    {
                        $query="select idioma, titol_es, texte_es from carta where idcarta = ".$this->id;
                    }
                    else
                    {
                        $query="select idioma, titol_cat, texte_cat from carta where idcarta = ".$this->id;
                    }    
                    ...
                    if ($resultat!=FALSE)
                    {
                        ...
                        if ($this->leng=='ES')
                        {
                          ...
                          $this->carta_descripcio=substr($row['texte_es'],0,200); 
                        }
                        else
                        {
                            $this->carta_titol=$row['titol_cat'];
                            $this->carta_descripcio=substr($row['texte_cat'],0,200);
                        }
                        if ($idioma_ent!='BOTH') $this->traduccio=FALSE;
                    }
                    $bd->desconectar();
      ...
      if (($this->section=='cronicas') || ($this->section=='croniques'))
			/* parametres de la secció concerts */
			{
				...
				if (($this->cronica_tipus!='main') && ($this->cronica_tipus!='entrada')) 
				{
					$this->cronica_tipus='main';	
				}
				if (isset($param['id']))
          ...
          if ($this->leng=='ES')
          {
              $query="select idioma, titol, cartell, intro_es from cronicas where idcronicas = ".$this->id;
          }
          else
          {
              $query="select idioma, titol, cartell, intro_cat from cronicas where idcronicas = ".$this->id;
          }                    
					$resultat=$bd->bd->query($query);
					if ($resultat!=FALSE)
					{
            ...
            if ($this->leng=='ES')
            {
                ...
                $this->cronica_descripcio=substr($row['intro_es'],0,200); 
            }
            else
            {
                $this->cronica_titol='Crónica de '.$row['titol'];
                $this->cronica_descripcio=substr($row['intro_cat'],0,200);
            }
		...
		public function translate_url($url)
		{
			...
			{		
					...
					if ((($this->section=='noticias') || ($this->section=='noticies')) && ($this->noticia_traduida!=''))
					{
						$this->noticia_traduida=urlencode($this->noticia_traduida);
						$url=str_replace(urlencode($this->noticia),$this->noticia_traduida,$url);
					}          
				}
		return $url;
		}
		...
		{            
			...
			if ($this->punter!=1) 
			{
				print '<a class="linkk" href="index.php?ln='.$this->leng.'&sec='.$this->section;
				if (($this->section=='concerts') || ($this->section=='conciertos'))
				{
        ...
        if (($this->section=='entrevistas') || ($this->section=='entrevistesn'))
				{
					print '&type='.$this->entrevista_tipus;
				}
        if (($this->section=='cronicas') || ($this->section=='croniques'))
        ...
        if (($this->section=='preferits') || ($this->section=='preferidos'))
				{
        ...
        if (($this->section=='cartaslector') || ($this->section=='carteslector'))
				{
					print '&type='.$this->carta_tipus;
				}
        if (($this->section=='preferidos') || ($this->section=='preferits'))
				...
				print '&pnt='.$punter_dir.'"><img class="iconavegador" src="pics/containers/prev_w.jpg" width="30" height="23" /></a>';
			}
        ...
        print '&pnt='.$punter_dir.'"><img class="iconavegador" src="pics/containers/next_w.jpg" width="30" height="23" /></a>';
			}                
            
			if ($numero>=($this->punter+$quantitat)) 
			{
				$punter_dir=((number_format(($numero/$quantitat), 0)*$quantitat)+1)-$quantitat;                
				print '<a class="linkk" href="index.php?ln='.$this->leng.'&sec='.$this->section;
				if (($this->section=='concerts') || ($this->section=='conciertos'))
				{
					print '&type='.$this->concert_tipus;
        ...
        print '&pnt='.$punter_dir.'"><img class="iconavegador" src="pics/containers/max_next_w.jpg" width="45" height="23" /></a>';
			}
			print '</div>';
		}
	}
	...
	class banner_700
	{
      ...
      //$this->banners[5]=array('ruta'=>'pics/banner/bvanner_hard.jpg','link'=>'http://www.satanarise.com/index.php?ln=ES&sec=concurso&tit=hardbuds','title'=>"Concurso Hard Buds",'target'=>'_new'); 
		...
		public function omplir($bd)
        {
            ...
            $query="select id, img, link, texte, datafi from banners where tipo = 700";
            ...
            if ($this->resultat_consulta!=FALSE) 
		    {
			    ...
			    if ($this->numero_resultats>0) 
			    {
                    ...
                    {    
                       ...
                       {
				        $this->banners[$x]=array('ruta'=>$resultat['img'],'link'=>$resultat['link'],'title'=>$resultat['texte'],'id'=>$resultat['id'],'target'=>'_new');
                      }  
			        }
		...
		{
			print '<a class="linkk" target="'.$this->banners[$this->publicar]['target'].'" href="index.php?ln=ES&sec=link&id='.$this->banners[$this->publicar]['id'].'" tittle="'.$this->banners[$this->publicar]['title'].'" ><img src="pics/banner/'.$this->banners[$this->publicar]['ruta'].'" width="700" height="100" alt="'.$this->banners[$this->publicar]['title'].'" /></a>';
			$this->publicar++;
			if ($this->publicar==$this->total_banners) $this->publicar=0;
		}
    ...
    class banner_100
    {
        ...
        public function omplir($bd)
        {
            ...
            $query="select id, img, link, texte, datafi from banners where tipo = 100";
            $this->resultat_consulta=$bd->query($query);
            if ($this->resultat_consulta!=FALSE) 
		    {
			    $this->numero_resultats=$this->resultat_consulta->num_rows;
      ...
      for ($i=0;$i<$total;$i++)
      {
      	print '<a class="linkk" target="'.$this->banners[$i]['target'].'" href="index.php?ln=ES&sec=link&id='.$this->banners[$i]['id'].'" tittle="'.$this->banners[$i]['title'].'" ><img src="pics/banner/'.$this->banners[$i]['ruta'].'" width="100" alt="'.$this->banners[$i]['title'].'" /></a><br /><br />';
      }			     
		}
