...
<!-- AddThis Button END -->
		...
		/* mostra les sugerencies de reviews que surten al final d'una review, per estil, per país i per mes visitades */
		{
			...
			$query="select reviews.link, reviews.banda, reviews.disc, reviews.portada, estil.estil from reviews left join estil on reviews.idestil=estil.idestil where reviews.idreviews <> ".$review->id." and estil.estil_global = ".$review->estil_global." order by reviews.data desc limit 0, 5";
			...
			$query="select reviews.link, reviews.banda, reviews.disc, reviews.portada, estil.estil 
from reviews 
left join estil on reviews.idestil=estil.idestil 
where reviews.idreviews <> ".$review->id." 
and reviews.data < ".$review->data."
order by reviews.vistes desc limit 0, 5";
			$this->resultat_consulta=$bd->query($query);
            ...
            if ($this->resultat_consulta!=FALSE) 
			{
				...
				if ($this->numero_resultats>0) 
				{
					...
					{
						case 'ES':
							print '<p>'.'Las más leídas del último año:'.'</p>';
						break;
						case 'CAT':
							print '<p>'.'Les més llegides del darrer any:'.'</p>';
						break;
					}
			...
			$query="select reviews.link, reviews.banda, reviews.disc, reviews.portada, estil.estil from reviews left join estil on reviews.idestil=estil.idestil where reviews.idreviews <> ".$review->id." order by reviews.vistes desc limit 0, 5";
			$this->resultat_consulta=$bd->query($query);
			...
			$query="select reviews.link, reviews.banda, reviews.disc, reviews.portada, estil.estil from reviews left join estil on reviews.idestil=estil.idestil where reviews.idpais=".$review->idpais." and reviews.idreviews <> ".$review->id." order by reviews.data desc limit 0, 5";
		...
		{
			$query= "select vistes from reviews where idreviews = $id";	
			$this->resultat_consulta=$bd->query($query);
			if ($this->resultat_consulta!=FALSE) 
			{
				$resultat=$this->resultat_consulta->fetch_assoc();
