<?php
	...
	class ob_noticias_web
	{
			...
			for ($i=0;$i<$this->numero_resultats;$i++)
			{
				...
				switch ($desti)
				{
					...
					case 'index':
						...
						if ($i==11) $i=$this->numero_resultats;
					break;
					case 'individual':
						if ($noticia->tipus==2)
						{
		...
		/* consulta noticies a la bbdd a partir d'una id */
		{
			...
			$error=TRUE;
			$query= "select news.idNews, type, descripcio, newscontent.Idioma, news.dateIn, Title, Body
			FROM news, newscontent
			WHERE newscontent.idNews = ".$id." and news.idNews = ".$id." and newscontent.Idioma='".$idioma."'";
			$this->resultat_consulta=$bd->query($query);
			if ($this->resultat_consulta!=FALSE) 
			{
				$this->numero_resultats=$this->resultat_consulta->num_rows;
		...
		{
			print '<meta property="og:image" content="'.$noticia->imatge1.'" />';
			...
			if ($noticia->control_imatges=='si') 
			{
				...
				print '<div class="centrar"><a href="'.$noticia->imatge1.'" rel="lightbox['.$noticia->id.']" title="'.$noticia->titol.'"><img src = "'.$noticia->imatge1.'" '.$tam_imp.' alt="'.$noticia->titol.'" /></a></div>';
...
<?php
		...
		public function mostrar_noticia_index ($noticia,$control_tags)
		{
			...
			if($titolAux != $noticia->titol) $titolAux .= "...";
			print '<h1><a class="linkk" href="index.php?ln='.$noticia->idioma.'&sec='.$section.'&id='.$noticia->id.'&noticia='.(urlencode($noticia->titol)).'"> '.$titolAux.'</a></h1>';
			$noticia->texte=$this->tractar_tag_link(2,$noticia->texte);
			print '<div class="contimgnot"><a href="index.php?ln='.$noticia->idioma.'&sec='.$section.'&id='.$noticia->id.'&noticia='.(urlencode($noticia->titol)).'" title="'.$noticia->titol.'">
				<img src = "'.$noticia->imatge1.'" alt="'.$noticia->titol.'" /></a></div>	';
			print '<p>'.substr($noticia->texte,0,115).'...</p></div>';
			if (($control_tags%4==3) || ($control_tags==($this->numero_resultats-1))) {
				print '</div>';
			}
		}
