<?php
	...
	class ob_cronicas_web
	{
                    ...
                    for ($x=1;$x<=5;$x++)
                    {
                        if ($x==1)
                        {
                            ...
                            if (($x==2) && ($cronica->$nom->imgs[$x]!=''))
                            {
                                ...
                                $texte=$texte.'<a href="pics/cronicas_pics/'.$cronica->$nom->imgs[$x].'" rel="lightbox" title="'.$cronica->titol.'"><img style="margin:2px" src = "pics/cronicas_pics/'.$cronica->$nom->imgs[$x].'" height="180" alt="'.$cronica->titol.'" /></a>';
                                $control_fi_div=TRUE;
		...
		{
			$query= "select visitas from cronicas where idcronicas = $id";	
			$this->resultat_consulta=$bd->query($query);
			if ($this->resultat_consulta!=FALSE) 
			{
				$resultat=$this->resultat_consulta->fetch_assoc();
