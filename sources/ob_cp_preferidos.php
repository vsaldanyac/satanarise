...
class ob_cp_preferidos
{
	...
	public function recull_parametres($formulari, $preferido, $basedades) /* Mira si hi ha un formulari enviat i recull parametres */
	{
		...
		if ($formulari['enviat'] == 'si') {
			...
			if ($preferido->nom == '') {
				...
				if (isset($formulari['pic']))
					$preferido->logo = trim($formulari['pic']);
					...
					/* Comprovacio erros al pujar */{
						switch ($_FILES['pic']['error']) {
							...
							case 4:
								break;
							default:
								$this->error = $this->error . 'Error al subir el archivo ' . $_FILES['pic'] . '.<br />';
								$this->formulari_ok = FALSE;
								break;
						}
						...
						if (stristr($_FILES['pic']['name'], '.gif') != FALSE)
							$ext = '.gif';
						if (stristr($_FILES['pic']['name'], '.png') != FALSE)
							$ext = '.png';
						if ($ext == '') {
			...
			if (($preferido->mes) < 10)
				$preferido->mes = '0' . $preferido->mes;
			$preferido->dia = $formulari['dia'];
			if (($preferido->dia) < 10)
				$preferido->dia = '0' . $preferido->dia;
	...
	/* controla fletxes de navegacio per presentar noticies */
	{
		...
		<div class="navegador">
			...
			<?php if ($punter != 1) {
				?>
				<a class="linkk" href="index_cp.php?sec=preferidos&action=edit&punter=1"><img class="ico_navegador"
