<?php
	class ob_cp_entrevnews {
		...
		public function recull_parametres($formulari,$entrevista) /* Mira si hi ha un formulari enviat i recull parametres */
		{
	       $entrevista->reset_entrevistes();
			if ($formulari['enviat']=='si') {
                ...
                if ($entrevista->timestamp=='') $time_file=(date('Y-m-d H:i:s'));
        				...
        				if (isset($_FILES['fitxer_logo']))
        				{
        					...
        					/* Comprovacio erros al pujar */
        					{
        						...
        						if ($ext=='') 
        						{
        							...
        							$directori='../pics/entrevistes_news_pics/'.$time_file.'_1'.$ext;
                ...
                for ($i=0;$i<3;$i++)
				{
					...
					/* Comprovacio erros al pujar */
					{
                        ...
                        if (isset($_POST['borrar_'.($i+1)])) {
                            print 'borrrrrar';
                            if (file_exists('../pics/entrevistes_news_pics/'.$entrevista->imgs[$i])) {
                                unlink('../pics/entrevistes_news_pics/'.$entrevista->imgs[$i]);
                            }                            
                            $entrevistas->imgs[$i]='borrar';
                        }
						...
						if ($ext=='') 
						{
							...
							$directori='../pics/entrevistes_news_pics/'.$time_file.'_'.($i+2).$ext;
                ...
                if (($entrevista->mes)<10) $entrevista->mes='0'.$entrevista->mes; 
                $entrevista->dia=$formulari['dia'];
                if (($entrevista->dia)<10) $entrevista->dia='0'.$entrevista->dia;
		...
		public function introduir($bs,$entrevista,$logica_id,$id)
		{
			...
			if ($this->resultat_consulta) 
			{
				...
				{ 
                    ...
                    $query="update entrevnews set link='".$entrevista->link."' where identrevistes=".$entrevista->id;
            ...
            if ($logica_id)
			{ 
                $query="select ruta, tipus, ordre from entrevnewsdata where identrevistes=".$entrevista->id." order by tipus, ordre asc";
                $this->resultat_consulta=$bs->query($query);
            ...
            if ($entrevista->logo!='')
			{
				...
				if ($logica_id) 
				{
                    ...
                    if ($entrevista->logo!=$data[0]['ruta']) {
                        ...
                        $query="update entrevnewsdata set ruta=".$entrevista->logo." where identrevistes=".$entrevista->id." and ordre=1 and tipus=1";
                        $this->resultat_consulta=$bs->query($query);
        				if ($this->resultat_consulta) 
        				{
                        ...
                        if (file_exists('../pics/entrevistes_news_pics/'.$data[0]['ruta'])) {
                            unlink('../pics/entrevistes_news_pics/'.$data[0]['ruta']);
                        }
                    } 
                    
				} else {
					$query="insert into entrevnewsdata (ordre, identrevistes, tipus, ruta) values (".$contador.", (select identrevistes from entrevnews order by identrevistes desc limit 1), '1', '".$entrevista->logo."')";
                    $this->resultat_consulta=$bs->query($query);
    				if ($this->resultat_consulta) 
    				{
    					print '<p class="terminal">¡¡Imagen logo añadida correctamente!!</p>';
                       ...
                       if (isset($data[$i])) {
                           if ($data[$i]['tipus']==1) { 
                               if ($entrevista->imgs[($i-1)]=='borrar') {
                                    $query="delete from entrevnewsdata where identrevistes=".$entrevista->id." and ordre=".$data[$i]['ordre']." and tipus=1";
                                    $contador=$contador-1;
                                   ...
                                   if ($entrevista->imgs[($i-1)]!=$data[$i]['ruta']) {
                                    ...
                                    $cont++;
                                   /* si es igual no es fa res.. sino... s'esborra i es posa un nou' */
                                       $query="update entrevnewsdata set ruta=".$entrevista->imgs[($i-1)]." where identrevistes=".$entrevista->id." and ordre=".$data[$i]['ordre']." and tipus=1";
                                       print $query;
                                        $this->resultat_consulta=$bs->query($query);
                    					if ($this->resultat_consulta) 
                    					{
                                       ...
                                       if (file_exists('../pics/entrevistes_news_pics/'.$data[$i]['ruta'])) {
                                           unlink('../pics/entrevistes_news_pics/'.$data[$i]['ruta']);
                                       }      
                                   }
                                }
                           }
                           ...
                           $cont=0; 
					       $query="insert into entrevnewsdata (ordre, identrevistes, tipus, ruta) values (".$contador.", '".$id."', '1', '".$entrevista->imgs[($i-1)]."')";
                           print $query;
                            $this->resultat_consulta=$bs->query($query);
        					if ($this->resultat_consulta) 
        					{
						...
						$query="insert into entrevnewsdata (ordre, identrevistes, tipus, ruta) values (".$contador.", (select identrevistes from entrevnews order by identrevistes desc limit 1), '1', '".$entrevista->imgs[($i-1)]."')";
                        print $query;
                        $this->resultat_consulta=$bs->query($query);
    					if ($this->resultat_consulta) 
    					{
			...
			if ($entrevista->tipus=='1') {
				...
				if ($logica_id) {
                        ...
                        if (isset($data[$i]))
                        {
                            if ($data[$i]['tipus']==2) { 
                                if ($entrevista->ruta_audio!=$data[$i]['ruta']) {
                                    $cont++;
                                    /* si es igual no es fa res.. sino... s'esborra i es posa un nou' */
                                    $query="update entrenewsdata set ruta=".$entrevista->imgs[($i-1)]." where identrevistes=".$entrevista->id." and ordre=".$data[$i]['ordre']." and tipus=2";
                                    $this->resultat_consulta=$bs->query($query);
                				    if ($this->resultat_consulta) 
                				    {   
					...
					$query="insert into entrevnewsdata (ordre, identrevistes, tipus, ruta) values (".$contador.", '".$id."', 2, '".$entrevista->ruta_audio."')";
                    $this->resultat_consulta=$bs->query($query);
    				if ($this->resultat_consulta) 
    				{
    					print '<p class="terminal">¡¡Audio añadido!!</p>';
					...
					$query="insert into entrevnewsdata (ordre, identrevistes, tipus, ruta) values (".$contador.", (select identrevistes from entrevnews order by identrevistes desc limit 1), 2, '".$entrevista->ruta_audio."')";
                    print $query;
                    $this->resultat_consulta=$bs->query($query);
    				if ($this->resultat_consulta) 
    				{
		...
		public function formulari($entrevista,$basedades)
		{
            ...
            if ($entrevista->logo!='')
            {
                print '<img src="../pics/entrevistes_news_pics/'.$entrevista->logo.'" width="325" /><br />';
			...
			for ($i=0;$i<3;$i++) 
			{
                ...
                {
                    print '<img src="../pics/entrevistes_news_pics/'.$entrevista->imgs[$i].'" width="100" /><br />';
		...
		/* consulta media a la bbdd a partir de la conexió, el numero d'inici de la consulta per data i la quantitat a mostrar */
		{
			...
			$query= "select identrevistes from entrevnews order by data desc limit ".$inici.", ".$quantitat." ";
		...
		{
			$entrevista->reset_entrevistes();
            $query= "select * from entrevnews where identrevistes = ".$id;
            ...
            $query= "select * from entrevnewsdata where identrevistes = ".$id." order by ordre asc";
			$this->resultat_consulta=$bd->query($query);
			$numero=$this->resultat_consulta->num_rows;
			if ($numero>=0) {
				for ($i=0;$i<$numero;$i++)					
				{
					...
					if ($resultat['tipus']==1) {
		          	    if ($i==0) {
                            ...
                            print 'Aixo es un logo: <img src="../pics/entrevistes_news_pics/'.$entrevista->logo.'" widht="100" /><br />';
                        } else {
                            $entrevista->imgs[($i-1)]=$resultat['ruta'];
                            print 'Aixo NO es un logo: <img src="../pics/entrevistes_news_pics/'.$entrevista->imgs[($i-1)].'" widht="100" /><br />';
                        }
		...
		public function eliminar_registre($bd,$id)
		{
			...
			$query="delete from entrevnews where identrevistes = '".$id."'";
			$this->resultat_consulta=$bd->query($query);
			if ($this->resultat_consulta) {
            ...
            $query="select ruta, tipus from entrevnewsdata where identrevistes = '".$id."'";
            $this->resultat_consulta=$bd->query($query);
			$numero=$this->resultat_consulta->num_rows;
            if ($numero>=0) {
				for ($i=0;$i<$numero;$i++)					
				{
					...
					if ($resultat['tipus']==1) {
		          	    if ($i==0) {
	       				    if (file_exists('../pics/entrevistes_news_pics/'.$resultat['ruta'])) {
                                unlink('../pics/entrevistes_news_pics/'.$resultat['ruta']);
                            }                   
                        }
            ...
            $query="delete from entrevnewsdata where identrevistes = '".$id."'";
			$this->resultat_consulta=$bd->query($query);
		...
		/* tenint l'objecte de la consulta les posa a pantalla */
		{
			...
			for ($i=0;$i<$this->numero_resultats;$i++) 
			{
				$row=$this->resultat_consulta->fetch_assoc();
				$entrevista->id=$row['identrevistes'];
				$query= "select banda, data from entrevnews where identrevistes=".$entrevista->id;
...
<?php			if ($punter!=1) {
?>					
                <a class="linkk" href="home_cp.php?sec=entrevistasn&action=edit&punter=1"><img class="ico_navegador" src="../pics/containers/max_prev.jpg" width="43" height="43" /></a>
<?php
				}
				if ($punter!=1) {
						$punter_dir=$punter-10;
					print '<a class="linkk" href="home_cp.php?sec=entrevistasn&action='.$action.'&punter='.$punter_dir.'"><img class="ico_navegador" src="../pics/containers/prev.jpg" width="43" height="43" /></a>';
				}

                if ($numero>=($punter+10)) {
						$punter_dir=$punter+10;
					print '<a class="linkk" href="home_cp.php?sec=entrevistasn&action='.$action.'&punter='.$punter_dir.'"><img class="ico_navegador" src="../pics/containers/next.jpg" width="43" height="43" /></a>';
				}                
                if ($numero>=($punter+10)) {
					$punter_dir=((number_format(($numero/$quantitat), 0)*$quantitat)+1)-10;
					print '<a class="linkk" href="home_cp.php?sec=entrevistasn&action='.$action.'&punter='.$punter_dir.'"><img class="ico_navegador" src="../pics/containers/max_next.jpg" width="43" height="43" /></a>';
				}
?>
            </div>    
<?php
