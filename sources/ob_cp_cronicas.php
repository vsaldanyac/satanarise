...
class ob_cp_cronicas
{
	...
	public function extract_id_section($bs, $cronicas)
	{
		$querycronicadata = "select idcronicasdata from cronicasdata where idcronicas=" . $cronicas->id . " order by idcronicasdata desc limit 1 ";
		$this->resultat_consulta3 = $bs->query($querycronicadata);
		if ($this->resultat_consulta3) {
			for ($x = 1; $x <= $cronicas->num_div; $x++) {
				$row = $this->resultat_consulta3->fetch_assoc();
				$nom = 'div' . $x;
	...
	public function introduir($bs, $cronicas, $logica_id, $id)
	{
		...
		if ($logica_id) {
			$query1 = "update cronicas set data='" . $cronicas->timestamp . "', cartell='" . $cronicas->cartell . "', titol='" . $cronicas->titol . "', intro_es='" . $cronicas->intro_es . "', intro_cat='" . $cronicas->intro_cat . "', idcolaborador1='" . $cronicas->idcolaborador1 . "', idcolaborador2='" . $cronicas->idcolaborador2 . "', idfoto1='" . $cronicas->idfoto1 . "', outro_cat='" . $cronicas->outro_cat . "', outro_es='" . $cronicas->outro_es . "', setlist='" . $cronicas->setlist . "' where idcronicas='" . $id . "'";
		} else {
		...
		if ($this->resultat_consulta) {
			...
			$this->extract_id_section($bs, $cronicas);
			...
			for ($x = 1; $x <= $cronicas->num_div; $x++) {
				...
				if (!$logica_id) {
					...
					$query1 = "update cronicasdata SET texte_cat = " . $cronicas->$nom->texte_cat . ", texte_es = " . $cronicas->$nom->texte_es . " where idcronicas=" . $id . " and idcronicasdata=" . $cronicas->$nom->id_cronica . "";
					print '<p class="terminal">';
					print $query1;
					print '</p>';
				}
				...
				if (($this->resultat_consulta2->num_rows) == 1) {
					...
					switch ($tasca) {
						case ('editar'):
							...
							print "<p class=\"titol\">Concierto: $cronicas->titol</p><br />";
