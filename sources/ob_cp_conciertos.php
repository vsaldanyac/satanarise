...
class cp_propers_concerts
{
		...
		if (isset($_POST['enviat_formulari_2'])) {
			if ($_POST['enviat_formulari_2'] == 'si') {
				...
				for ($i = 1; $i <= $this->num_concerts; $i++) {
					$nom = 'concert' . $i;
					if ($this->dades->id != '')
						$this->dades->$nom->id = $this->dades->id;
					...
					for ($y = 1; $y <= $this->num_bandes; $y++) {
						$nom2 = 'banda' . $y;
						if ($this->dades->id != '')
							$this->dades->$nom->$nom2->id = $this->dades->id;
	...
	/* controla fletxes de navegacio per presentar noticies */
	{
		...
		<div class="navegador">
			...
			<?php if ($punter != 1) {
				?>
				<a class="linkk" href="home_cp.php?sec=conciertos&action=edit&punter=1"><img class="ico_navegador"
