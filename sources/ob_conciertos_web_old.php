			...
			for ($i=0;$i<$this->numero_resultats;$i++)
			{
				...
				switch ($this->concert_data->tipus)
				{
					...
					/* Concierto único*/
						...
						print '<img src="pics/conc/'.$this->concert_data->cartell.'" height=150" alt="'.$this->concert_data->grups.'" />';
					...
					/* Gira */
						...
						{
							print $this->concert_data->cartell;
						} else {
							print $this->concert_data->cartell_concert;
						}
						print '" height=150" alt="'.$this->concert_data->nom.'" />';
						$this->concert_data->dataIn=$this->timestamp_a_data($this->concert_data->dateIn);
						print '<div class="data_con">'.$this->concert_data->dataIn.'</div>';
					...
					/* Festival */
						...
						print '<img src="pics/conc/'.$this->concert_data->cartell.'" height=150" alt="'.$this->concert_data->nom.'" />';
						$this->concert_data->dataIn=$this->timestamp_a_data($this->concert_data->dateIn);
						print '<div class="data_con">'.$this->concert_data->dataIn.'</div>';
			...
			if ($this->resultat_consulta!=FALSE) 
			{
				...
				for ($i=0;$i<$this->numero_resultats;$i++)
				{
					...
					if ($cartell_especific=='')
					{
						...
						print $cartell_especific;
					}
					

					
					print '" alt="'.$this->concert_data->grups.' '.$this->concert_data->sala.' ('.$this->concert_data->localitat.')" height="154"/>';
					print '<h1>';
					switch ($this->concert_data->tipus){
						case '1':
							if ($this->concert_data->nom!='')
...
<!-- AddThis Button END -->';
			print '</div>';
				...
				case '2':
					if ($this->dades->cartell!='') 
					{ /* Hi ha un cartell unic per la gira */
					...
					for ($y=0;$y<$this->dades->num_bandes;$y++)
				 	{
						...
						if ($this->dades->$nom->$nom2->video!='') 
						{
							...
							{
								if ($grups[$z]==$this->dades->$nom->$nom2->nom) $coincidencia=TRUE;								
							}
							if (!$coincidencia) 
							{
