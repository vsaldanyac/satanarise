<?php
	...
	class ob_entrevnews_web
	{
		...
		/* consulta noticies a la bbdd a partir de la conexió, el numero d'inici de la consulta per data i la quantitat a mostrar */
		{
			...
			$query= "select identrevistes, banda, link, titol_es, titol_cat, data from entrevnews where idioma = '".$leng."' or idioma = 'BOTH' order by data desc limit ".$inici.", ".$quantitat;
        ...
        public function mostrar_entrevistes_per_data_entrada_lateral($bd,$leng)
		{
			$entrevista  = new ob_entrevnews;
			for ($i=0;$i<$this->numero_resultats;$i++)
			{
      ...
      {
          case 'ES':
              $query= "select entrevnews.identrevistes, entrevnews.banda, entrevnews.link, entrevnews.titol_es, entrevnews.tipus, entrevnews.texte_es, entrevnews.idcolaboradors, entrevnews.idcolaboradors2, entrevnews.idtraductor, entrevnews.data from entrevnews where identrevistes=".$id;
          break;
          case 'CAT':
              $query= "select entrevnews.identrevistes, entrevnews.banda, entrevnews.link, entrevnews.titol_cat, entrevnews.tipus, entrevnews.texte_cat, entrevnews.idcolaboradors, entrevnews.idcolaboradors2, entrevnews.idtraductor, entrevnews.data from entrevnews where identrevistes=".$id;
          break;
      }
      ...
      if ($this->resultat_consulta==FALSE) 
            ...
            /* extraccio imatges */
      $query= "select * from entrevnewsdata where identrevistes=".$id." order by ordre";            
			$this->resultat_consulta2=$bd->query($query);
      ...
      print '<img src="pics/entrevistes_news_pics/'.$entrevista->logo.'" width="300" alt="'.$entrevista->banda.'" /></div>';
      ...
      /* tags 
          ...
          [img#] ---------> <a href="pics/entrevistes_news_pics/ruta" rel="lightbox[lr]" title="Banda"><img class="esq" src = "pics/entrevistes_news_pics/ruta" width="400" alt="Banda" /></a>
          [imgcentrada#] ->
      */
      ...
      for ($y=0;$y<3;$y++)
      {
          ...
          if ($entrevista->imgs[$y]!='')
          {
              $link_img[$y]='<div class="centrar"><a href="pics/entrevistes_news_pics/'.$entrevista->imgs[$y].'" rel="lightbox[.$entrevista->banda.]" title=".$entrevista->banda."><img src="pics/entrevistes_news_pics/'.$entrevista->imgs[$y].'" width="400" alt="'.$entrevista->banda.'" /></a></div><br />';
          }
      }
      ...
      {
          case 'ES':
              $query="select banda, identrevistes, link, titol_es, data from entrevnews where (idioma = 'ES' OR idioma = 'BOTH') AND (identrevistes <> ".$entrevista->id.") order by visites desc limit 0, 5";
          break;
          case 'CAT':
              $query="select banda, identrevistes, link, titol_cat, data from entrevnews where (idioma = 'CAT' OR idioma = 'BOTH') AND (identrevistes <> ".$entrevista->id.") order by visites desc limit 0, 5";
          break;
      }
      ...
      if ($this->resultat_consulta!=FALSE) 
				...
				for ($x=0;$x<$this->numero_resultats;$x++)
        {
          ...
          $query="select ruta from entrevnewsdata where identrevistes=".$identrevistes." and tipus=1 and ordre=1";
          $this->resultat_consulta2=$bd->query($query);
          if ($this->resultat_consulta2!=FALSE) 
          { 
            $row=$this->resultat_consulta2->fetch_assoc();
   	...
   	public function afegir_visita ($bd,$id)
		{
			$query= "select visites, banda from entrevnews where identrevistes = $id";	
			$this->resultat_consulta=$bd->query($query);
			if ($this->resultat_consulta!=FALSE) 
			{
			$resultat=$this->resultat_consulta->fetch_assoc();
            ...
            $query="update entrevnews set visites=$vistes where identrevistes=$id";
            $this->resultat_consulta=$bd->query($query);
            if ($this->resultat_consulta!=FALSE) 
			 {}
			}
